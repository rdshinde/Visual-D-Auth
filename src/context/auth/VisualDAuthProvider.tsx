...
const AuthFormContext = createContext<{
  ...
  isLoading: false,
  contractMethodResponseHandler: () => null,
  userMnemonicPhrase: [],
});
...
export const useAuthProvider = (): UseAuthProvider => useContext(AuthFormContext);
...
export const VisualDAuthProvider = ({
  ...
  const [authFormState, authFormDispatch] = useReducer(authFormReducer, initialAuthFormState);
  ...
  useEffect(() => {
    if (authFormState.pwdImages.length > 0 && authFormState.username) {
      authFormDispatch({
        ...
        payload: getPasswordHash(authFormState.pwdImages, authFormState.username),
      });
    }
  }, [authFormState.pwdImages, authFormState.username]);
  ...
  useEffect(() => {
    if (isOnlySixImagesInPwd(authFormState.pwdImages) && authFormState.username) {
      authFormDispatch({
    ...
    if (chosenRoute === RouteNames.REGISTER) {
      switch (currentStep) {
        ...
        case StepNames.PASSWORD:
          if (pwdHash && username && isOnlySixImagesInPwd(authFormState.pwdImages) && mnemonicPhrase) {
            response = fetchContractMethod(
      ...
      switch (currentStep) {
        case StepNames.USERNAME:
          ...
          break;
        case StepNames.PASSWORD:
          if (pwdHash && username && isOnlySixImagesInPwd(authFormState.pwdImages)) {
            response = fetchContractMethod(
                ...
                if (usernameRes?.isUsernameTaken) {
                  ...
                  if (mnemonicPhraseResponse) {
                    ...
                    if (mnemonicPhraseRes) {
                      setUserMnemonicPhrase([...mnemonicPhraseRes.mnemonicPhrase.split(' ')]);
                      uiDispatch({
                        type: UiActionsTypes.GO_TO_NEXT_STEP,
                        payload: allSteps[currentStepIndex + 1].stepName || '',
                      });
  ...
  useEffect(() => {
    if (onSuccess && onSuccess.status) {
      onSuccessHandler
        ...
        : toast.error('Please provide the onSuccessHandler function in VisualDAuthProvider');
    }
  ...
  useEffect(() => {
    if (onError && onError.status) {
      onErrorHandler
        ...
        : toast.error('Please provide the onErrorHandler function in VisualDAuthProvider');
    }
